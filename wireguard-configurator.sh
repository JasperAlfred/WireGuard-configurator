...
INSTOS_ENG="---=== This installer working only on a: Debian, Ubuntu ===---"
INSTOS_RUS="---=== Этот установщик работает только в операционках: Debian, Ubuntu ===---"
...
SSHPORT_ENG="Server's SSH port !!!WRITE IT DOWN!!!"
SSHPORT_RUS="Порт SSH для подключения к серверу !!!ЗАПИСАТЬ!!!"
...
WGRUNNINGTEXT_RUS="\n`echo ${COLORRED}`ВНИМАНИЕ: WireGuard похоже не запущен.`echo ${COLOREND}` \n `echo ${COLORORANGE}`Вы можете проверить запущен ли WireGuard следующей командной: systemctl status wg-quick@`echo ${SERVER_WG_NIC}${COLOREND}` \n`echo ${COLORORANGE}`Если в выводе приветствует \"Cannot find device `echo ${SERVER_WG_NIC}`\", пожалуйста перезагрузите сервер!`echo ${COLOREND}`"
...
function adminSurvey() {	
	...
	until [[ ${CLIENT_DNS_2} =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
		...
		RANDOM_SSH_PORT=$(shuf -i21000-39999 -n1)
	until [[ ${SERVER_SSH_PORT} =~ ^[0-9]+$ ]] && [ "${SERVER_SSH_PORT}" -ge 1 ] && [ "${SERVER_SSH_PORT}" -le 40000 ]; do		
		read -rp "`eval echo $\`echo SSHPORT_$TLANG\`` [20000-40000]: " -e -i "${RANDOM_SSH_PORT}" SERVER_SSH_PORT
	done
...
function installWG() {
	...
	if [[ ${OS} == 'ubuntu' ]] || [[ ${OS} == 'debian' && ${VERSION_ID} -gt 10 ]]; then
		apt-get update
		apt-get install -y wireguard iptables resolvconf qrencode ufw
	elif [[ ${OS} == 'debian' ]]; then
		...
		apt update
		apt install -y iptables resolvconf qrencode ufw
...
SERVER_SSH_PORT=${SERVER_SSH_PORT}" > /etc/wireguard/params
...
net.ipv6.conf.all.forwarding = 1" > /etc/sysctl.d/wg.conf
	...
	sed -i "s/.*Port.*/Port ${SERVER_SSH_PORT}/" /etc/ssh/sshd_config && sed -i "s/.*SyslogFacility.*/SyslogFacility AUTH/" /etc/ssh/sshd_config && sed -i "s/.*LogLevel.*/LogLevel QUIET/" /etc/ssh/sshd_config && service ssh restart
	rm /var/log/auth.log*
	sed -i "s/.*\& stop.*/\& stop/" /etc/rsyslog.d/20-ufw.conf && ufw logging off && rm /var/log/ufw.log* && ufw allow ${SERVER_SSH_PORT}/tcp && ufw allow ${SERVER_PORT}/tcp && ufw --force enable
	echo "0 * * * * cat /dev/null >/var/log/wtmp
0 * * * * cat /dev/null >/var/log/btmp" |crontab
}
